...
void karte_t::recalc_season_snowline(bool set_pending)
{
	...
	const sint8 winterline = settings.get_winter_snowline();
	const sint8 summerline = max(settings.get_climate_borders(arctic_climate,1),settings.get_climate_borders(arctic_climate,0));
	snowline = summerline - (sint8)(((summerline-winterline)*factor)/100);
...
//				citizens = citizens/10+simrand(2*citizens+1);
				...
				while(  current_bev < citizens  ) {
					...
					current_bev += growth;
					cities[i]->change_size( growth, new_town );
...
void karte_t::init(settings_t* const sets, sint8 const* const h_field)
{
	...
	max_allowed_height = sets->get_maximumheight();
	min_allowed_height = sets->get_minimumheight();
...
void karte_t::step()
{
	...
	for (sint32 i = (sint32)convoi_array.get_count(); i-- > 0; ) {
...
DBG_MESSAGE("karte_t::load()", "laden_abschliesen for tiles finished" );
	...
	weighted_vector_tpl<stadt_t*> new_cities(cities.get_count() + 1);
	for(stadt_t* const s : cities) {
		s->finish_rd();
		s->recalc_target_cities();
		new_cities.append(s, s->get_einwohner());
		INT_CHECK("simworld 1278");
	}
	swap(cities, new_cities);
...
DBG_MESSAGE("karte_t::load()", "%d factories loaded", fab_list.get_count());
	...
	if(file->is_version_less(99, 14)) {
		...
		settings.set_factory_worker_maximum_towns(cities.get_count() + 1);
		for(stadt_t* const i : cities) {
			i->verbinde_fabriken();
		}
...
void karte_t::rdwr_gamestate(loadsave_t *file, loadingscreen_t *ls)
{
	...
	if (file->is_loading()) {
		...
		max_allowed_height = settings.get_maximumheight();
		min_allowed_height = settings.get_minimumheight();
...
const vector_tpl<const goods_desc_t*> &karte_t::get_goods_list()
{
	if (goods_in_game.empty()) {
		...
		for(fabrik_t* const factory : get_fab_list()) {
			slist_tpl<goods_desc_t const*>* const produced_goods = factory->get_produced_goods();
			for(goods_desc_t const* const good : *produced_goods) {
